...
const internalNavigation=new URLSearchParams(location.search).get('from')==='nav';
if(internalNavigation){const cleanUrl=new URL(location.href);cleanUrl.searchParams.delete('from');history.replaceState(null,'',`${cleanUrl.pathname}${cleanUrl.search}${cleanUrl.hash}`)}
let introClosing=false;
function closeIntro(immediate=false){
  if(introClosing)return;
  introClosing=true;
  // Arm the reveal before lifting the loading state so the header never flashes early.
  document.body.classList.add('hero-ready');
  document.body.classList.remove('is-loading');
...
const acReceptionTopics={
  ...
  booking:{terms:['buchen','buchung','reservieren','reservation','booking','book','boeken','reserveren','chalet available','room available','chalet verfugbar','zimmer verfugbar','chalet beschikbaar','kamer beschikbaar'],action:'booking'},
  price:{terms:['preis','preise','kosten','price','prices','cost','rate','tarif','prijs','prijzen'],action:'booking'},
  parking:{terms:['park','parken','parkplatz','tiefgarage','garage','auto','parking','car','parkeren'],action:'location'},
  sauna:{terms:['sauna','wellness','badezimmer','dusche','bathroom','shower','badkamer','douche'],action:'chalets'},
  skiRoom:{terms:['skiraum','ski room','storage room','berging','skiruimte'],action:'chalets'},
  capacity:{terms:['personen','gaste','people','persons','guests','capacity','kapazitat','gasten'],action:'chalets'},
  restaurant:{terms:['restaurant','gutshof','essen','abendessen','dinner','food','eten','reservierung'],action:'restaurant'},
  location:{terms:['lage','adresse','anfahrt','entfernung','location','address','directions','distance','ligging','waar','where','8er jet','lift'],action:'location'},
  summerCard:{terms:['summer card','sommer card','sommercard','sommerkarte','zomerkaart','zomer card'],action:'summer'},
  winter:{terms:['winter','sunweb','ski','schnee','skipiste','snow','piste'],action:'winter'},
  bread:{terms:['brotchen','brotchenservice','fruhstuck','fruhstucksbrotchen','brot','bread','bread roll','bread rolls','bread service','breakfast','breakfast roll','breakfast rolls','rolls','brood','broodjes','broodjesservice','broodservice','ontbijt'],action:'comfort'},
  laundry:{terms:['wasche','waschen','waschmaschine','trockner','laundry','washing machine','dryer','wasmachine','droger'],action:'comfort'},
  wifi:{terms:['wlan','wi fi','wifi','internet'],action:'contact'},
  pets:{terms:['hund','hunde','haustier','pet','pets','dog','dogs','huisdier','hond','honden'],action:'contact'},
  playground:{terms:['kind','kinder','spielplatz','playground','children','kids','speeltuin','kinderen'],action:'comfort'},
  contact:{terms:['kontakt','telefon','anrufen','email','e mail','rezeption','contact','phone','call','reception','receptie','bellen'],action:'contact'}
};
const acReceptionUi={
...
function acReceptionSimilarity(a,b){
  ...
  for(let i=1;i<=b.length;i++)for(let j=1;j<=a.length;j++)matrix[i][j]=Math.min(matrix[i-1][j]+1,matrix[i][j-1]+1,matrix[i-1][j-1]+(a[j-1]===b[i-1]?0:1));
  return 1-matrix[b.length][a.length]/Math.max(a.length,b.length);
}
function acReceptionStem(word){
  const endings=['service','ungen','ung','eren','ieren','en','ern','er','es','e','ing','ed','ly','ies','s','jes','je'];
  for(const ending of endings){
    if(word.length-ending.length>=4&&word.endsWith(ending))return word.slice(0,-ending.length);
  }
  return word;
}
function acReceptionWordMatch(token,term){
  if(token===term)return true;
  if(token.length>=5&&term.length>=5&&(token.startsWith(term)||term.startsWith(token)))return true;
  const tokenStem=acReceptionStem(token),termStem=acReceptionStem(term);
  if(tokenStem.length>=4&&termStem.length>=4&&(tokenStem===termStem||tokenStem.startsWith(termStem)||termStem.startsWith(tokenStem)))return true;
  return acReceptionSimilarity(token,term)>=.76;
}
function acReceptionScores(message){
  const normalized=normalizeConciergeText(message).replace(/[^\p{L}\p{N}]+/gu,' ').trim();
  const tokens=normalized.split(/\s+/).filter(Boolean);
  return Object.entries(acReceptionTopics).map(([key,topic])=>{
    let score=0;
    let phraseMatched=false;
    const matchedTokens=new Set();
    topic.terms.forEach(term=>{
      const clean=normalizeConciergeText(term);
      const termTokens=clean.split(' ');
      if(termTokens.length>1&&!phraseMatched&&normalized.includes(clean)){score+=6;phraseMatched=true;return}
      if(termTokens.length>1&&!phraseMatched&&termTokens.every(part=>tokens.some(token=>acReceptionWordMatch(token,part)))){score+=5;phraseMatched=true;return}
      if(termTokens.length===1){
        let index=tokens.findIndex((token,i)=>!matchedTokens.has(i)&&token===clean);
        let points=4;
        if(index<0){index=tokens.findIndex((token,i)=>!matchedTokens.has(i)&&acReceptionWordMatch(token,clean));points=2}
        if(index>=0){matchedTokens.add(index);score+=points}
      }
    });
    if(acReceptionLastTopics.includes(key)&&/\b(und|auch|dazu|it|that|also|and|what about|en|ook|daarover)\b/.test(normalized))score+=3;
    return{key,score};
  }).filter(item=>item.score>=2).sort((a,b)=>b.score-a.score);
}
function acReceptionResolve(message){
  const lang=detectConciergeLanguage(message);
  const normalized=normalizeConciergeText(message).trim();
  const simple=pickConciergeAnswer(message);
  const isConversation=/wie geht|how are|hoe gaat|danke|thanks|bedankt|tschuss|goodbye|tot ziens|^hallo[!. ]*$|^hello[!. ]*$|^hi[!. ]*$|hilfe|help|was kannst|what can/.test(normalized);
  if(isConversation&&simple!==conciergeAnswers[lang].fallback)return{lang,text:simple,actions:[]};
  let matches=acReceptionScores(message);
  if(!matches.length&&acReceptionLastTopics.length&&/\b(wie viele|how many|hoeveel|und|and|en|auch|also|ook|dazu|that|daarover)\b/.test(normalized))matches=acReceptionLastTopics.map(key=>({key,score:2}));
  const threshold=Math.max(2,(matches[0]?.score||0)-2);
  const selected=matches.filter(item=>item.score>=threshold).slice(0,3).map(item=>item.key);
  if(!selected.length)return{lang,text:acReceptionUi[lang].unknown+'\n+43 6457 33971 · info@alpenchalets.at',actions:['contact']};
  acReceptionLastTopics=selected;
  return{
    lang,
    text:selected.map(key=>conciergeAnswers[lang][key]).filter(Boolean).join('\n\n'),
    actions:[...new Set(selected.map(key=>acReceptionTopics[key].action))].slice(0,2)
  };
}
function acReceptionSeasonalHref(href){
  if(/^https?:/.test(href)||href.includes('#'))return href;
  const season=new URLSearchParams(location.search).get('season');
  return season?href+'?season='+encodeURIComponent(season):href;
}
function acReceptionAddMessage(text,type='bot',actions=[],lang=document.documentElement.lang){
  if(!butlerMessages)return;
  const bubble=document.createElement('div');
  bubble.className='butler-bubble '+(type==='user'?'is-user':'is-bot');
  const copy=document.createElement('span');copy.textContent=text;bubble.appendChild(copy);
  if(type==='bot'&&actions.length){
    const actionBar=document.createElement('div');actionBar.className='butler-actions';
    actions.forEach(action=>{
      const link=document.createElement('a');
      const actionUrl=acReceptionActionUrls[action];
      link.href=acReceptionSeasonalHref(actionUrl);
      link.textContent=acReceptionUi[lang]?.actions[action]||action;
      if(/^https?:/.test(actionUrl)){link.target='_blank';link.rel='noopener noreferrer'}
...
function acReceptionSuggestionGroup(){
  const page=location.pathname.split('/').pop()||'index.html';
  if(/restaurant/.test(page))return'restaurant';
  if(/winter/.test(page)||new URLSearchParams(location.search).get('season')==='winter')return'winter';
  if(/chalet/.test(page))return'chalet';
  return'default';
}
function acReceptionRenderSuggestions(){
  if(!butlerMessages)return;
  butlerMessages.querySelector('.butler-suggestions')?.remove();
  const lang=(document.documentElement.lang in acReceptionUi)?document.documentElement.lang:'de';
  const wrap=document.createElement('div');wrap.className='butler-suggestions';
  const label=document.createElement('span');label.textContent=acReceptionUi[lang].suggest;wrap.appendChild(label);
  acReceptionSuggestions[acReceptionSuggestionGroup()][lang].forEach(question=>{
    const button=document.createElement('button');button.type='button';button.textContent=question;
    button.addEventListener('click',()=>{butlerInput.value=question;butlerForm.requestSubmit()});
    wrap.appendChild(button);
  });
  butlerMessages.appendChild(wrap);
}
if(butlerForm&&butlerInput&&butlerMessages){
  butlerForm.addEventListener('submit',event=>{
    event.preventDefault();event.stopImmediatePropagation();
    const message=butlerInput.value.trim();if(!message)return;
    butlerMessages.querySelector('.butler-suggestions')?.remove();
    acReceptionAddMessage(message,'user');butlerInput.value='';
    window.setTimeout(()=>{const result=acReceptionResolve(message);acReceptionAddMessage(result.text,'bot',result.actions,result.lang)},220);
  },true);
  acReceptionRenderSuggestions();
  langButtons.forEach(button=>button.addEventListener('click',()=>window.setTimeout(acReceptionRenderSuggestions,0)));
}
