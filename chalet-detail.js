...
document.querySelectorAll('.amenity').forEach((card,index)=>{
  ...
  icon.innerHTML=amenityIconPaths[index%amenityIconPaths.length];
  card.prepend(icon);
});
...
function readSavedLanguage(){try{const value=localStorage.getItem(languageStorageKey);if(value)return value}catch{}const match=document.cookie.match(/(?:^|;\s*)alpenchalets-language=(de|en|nl)(?:;|$)/);return match?match[1]:'de'}
