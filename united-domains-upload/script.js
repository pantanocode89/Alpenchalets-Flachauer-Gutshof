(() => {
  ...
  window.addEventListener('error', () => setTimeout(releaseIntro, 200), {once:true});
  setTimeout(releaseIntro, 3200);
})();
...
let slideIndex=0;
function updateHeader(){const scrolled=window.scrollY>30;header.classList.toggle('scrolled',scrolled);backToHero.classList.toggle('is-visible',window.scrollY>360)}
...
function updateMobileBackToTop(){const visible=window.scrollY>450&&document.documentElement.scrollHeight>innerHeight*1.35;mobileBackToTop.classList.toggle('is-visible',visible);document.body.classList.toggle('mobile-back-active',visible)}
...
let heroTimer;
function showTimedHero(){if(!slides.length)return;const hour=new Date().getHours();const period=hour>=6&&hour<18?'day':'night';let next=slides.findIndex(slide=>slide.dataset.period===period);if(next<0)next=0;slides.forEach((slide,index)=>slide.classList.toggle('active',index===next));slideIndex=next;if(slideNumber)slideNumber.textContent=period==='day'?'DAY':'NIGHT'}
...
function closeIntro(){document.body.classList.remove('is-loading');document.documentElement.classList.add('intro-seen');if(siteIntro){siteIntro.classList.add('is-hidden');setTimeout(()=>siteIntro.remove(),900)}}
function hasSeenIntro(){try{return sessionStorage.getItem(introKey)==='true'||window.name.includes(introKey)}catch{return true}}
...
if(!siteIntro||reduceMotion||hasSeenIntro()){closeIntro()}else{markIntroSeen();window.addEventListener('load',()=>setTimeout(closeIntro,650),{once:true});setTimeout(closeIntro,1100)}
